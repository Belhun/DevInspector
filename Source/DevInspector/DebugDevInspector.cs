...
{
    public static class DebugDevInspector
    {
        ...
        public static void SetLogging(string className, bool isEnabled)
        {
            ...
            {
                Verse.Log.Warning($"<color=#00FF00>[Dev Inspector] Attempted to set logging for unknown class/feature:</color> {className}");
            }
        }
        ...
        public static void PrintLoggingStatus()
        {
            ...
            {
                Verse.Log.Message($"[Dev Inspector] Logging for {entry.Key}: {(entry.Value ? "Enabled" : "Disabled")}");
            }
        }
